...
if(command_rx_val==42)
                                ...
                                PerformDeviceApplicationErase();                                                                                                                                     //удаляем штатную прошвку нафиг, доступно только из хексхартлодера
                                }
if(command_rx_val==43){                    //запись ацп значения 1й точки калибровки первичной переменной в еепром
                        #asm ("cli")            
...
if(command_rx_val==111){                //запись ацп значения 2й точки калибровки первичной переменной в еепром
                        #asm ("cli")
...
//        }
         ...
         if(rangeIndexEepPB2!= rangeIndexEepPB1)      //одна из ячеек 1 или 2 содержит ошибочную информацию
        {               
                 
        if(rangeIndexEep==rangeIndexEepPB2)
            return 1;           //0я и 2я ячейки верные, а 1я - неверная  
        if(rangeIndexEep==rangeIndexEepPB1)
            return 2;           //0я и 1я ячейки верные, а 2я ячейка некорректна     
            return 3;           //самый печальный исход, все 3 ячейки убитые
        } 
//        secondPoint = 0x23;                  
       return 0;   
...
char checkCalibrationFlagValidity()              //приходим сюда уже зная, что калибровка была
{
...
//         tmpCCCPB2 = (char*)&CalibrationConfigChangedPB2;
        if(CalibrationConfigChangedPB2!= CalibrationConfigChangedPB1)      //одна из ячеек 1 или 2 содержит ошибочную информацию
        {               
...
//        secondPoint = 0x23;                  
       return 0;                  //2я и 1я ячейки корректны
}

char checkCalibrationRatesValidity(char index)
{
              if(ADC_PV_calibration_point1[index]==ADC_PV_calibration_point1_PB1[index])                //проверяем 1ю точку калибровки выбранного диапазона
        {
                if(ADC_PV_calibration_point1[index]!=ADC_PV_calibration_point1_PB2[index])//неисправна ячейка ЕЕПРОМ PB2 
                       return 1;     
        }
        else
        {
                if(ADC_PV_calibration_point1[index]==ADC_PV_calibration_point1_PB2[index])
                        return 2;    //неисправна ячейка ЕЕПРОМ PB1
                else if(ADC_PV_calibration_point1_PB1[index]==ADC_PV_calibration_point1_PB2[index])      
                {
                               return 3;// ADC_PV_calibration_point1[index] = ADC_PV_calibration_point1_PB1[index];      //неисправна ячейка ЕЕПРОМ point1
                             //   CalculateCalibrationRates();
                }                                       
          }
        if(ADC_PV_calibration_point2[index]==ADC_PV_calibration_point2_PB1[index])                //проверяем 2ю точку калибровки выбранного диапазона
        {
                if(ADC_PV_calibration_point2[index]!=ADC_PV_calibration_point2_PB2[index])//неисправна ячейка ЕЕПРОМ PB2 
                       return 4;    
        }
        else
        {
                if(ADC_PV_calibration_point2[index]==ADC_PV_calibration_point2_PB2[index])
                       return 5;    //неисправна ячейка ЕЕПРОМ PB1
                else if(ADC_PV_calibration_point2_PB1[index]==ADC_PV_calibration_point2_PB2[index])      
                {
                                return 6;      //неисправна ячейка ЕЕПРОМ point1
                               // CalculateCalibrationRates();
                }                                       
        }
        
        if(calibrationKeep[index]==calibrationKeep_PB1[index])                                        //проверяем калибровочный коэффициент К
        {
                if(calibrationKeep[index]!=calibrationKeep_PB2[index])//неисправна ячейка ЕЕПРОМ PB2 
                        return 7;      
        }
        else
        {
                if(calibrationKeep[index]==calibrationKeep_PB2[index])
                        return 8;    //неисправна ячейка ЕЕПРОМ PB1
                else if(calibrationKeep_PB1[index]==calibrationKeep_PB2[index])
                {
                                return 9;      //неисправна ячейка ЕЕПРОМ point1
                                //CalculateCalibrationRates();
                }                                       
        }  
        
        if(calibrationBeep[index]==calibrationBeep_PB1[index])                                        //проверяем калибровочный коэффициент B
        {
                if(calibrationBeep[index]!=calibrationBeep_PB2[index])//неисправна ячейка ЕЕПРОМ PB2 
                       return 10;      
        }
        else
        {
                if(calibrationBeep[index]==calibrationBeep_PB2[index])
                        return 11;    //неисправна ячейка ЕЕПРОМ PB1
                else if(calibrationBeep_PB1[index]==calibrationBeep_PB2[index])
                {
                                return 12;      //неисправна ячейка ЕЕПРОМ point1
                                //CalculateCalibrationRates();
                }                                       
        } 
        return 0;          
...
{
if(Parameter_bank[2]!=0xb3)//код тип устройства производителя
      Parameter_bank[2]=0xb3;
if(Parameter_bank[6]!=0x01)//версия
    Parameter_bank[6]=0x01; 
if(Parameter_bank[11]!=0xBC)//старший бит КС
    Parameter_bank[11]=0xBC; 
if(Parameter_bank[10]!=0xBF)//младший бит КС
...
//       tmpcrc = (int)(Parameter_bank[10]<<8)|Parameter_bank[11];
//         if(tmpDevCode==tmpDevCodePB1)                //проверяем 1ю точку калибровки выбранного диапазона
//        {
//                if(tmpDevCode!=tmpDevCodePB2)//неисправна ячейка ЕЕПРОМ PB2  
//                          EEPROM_write(0x001AF, tmpDevCode);//     devCodePB2 = Parameter_bank[2];      
//        }
//        else
//        {
//                if(tmpDevCode==tmpDevCodePB2)     
//                        EEPROM_write(0x001AE, tmpDevCode);//devCodePB1 = Parameter_bank[2];    //неисправна ячейка ЕЕПРОМ PB1
//                else if(tmpDevCodePB1==tmpDevCodePB2)      
//                {  
//                                Parameter_bank[2] = tmpDevCodePB1;      //неисправна ячейка ЕЕПРОМ point1
//                               // CalculateCalibrationRates();
//                }                                       
//          }    
//        if(Parameter_bank[6]==firmwareRevPB1)                //проверяем 2ю точку калибровки выбранного диапазона
//        {
//                if(Parameter_bank[6]!=firmwareRevPB2)//неисправна ячейка ЕЕПРОМ PB2  
//                        firmwareRevPB2 = Parameter_bank[6];      
//        }
//        else
//        {
//                if(Parameter_bank[6]==firmwareRevPB2)        
//                       firmwareRevPB1 = Parameter_bank[6];    //неисправна ячейка ЕЕПРОМ PB1
//                else if(firmwareRevPB1==firmwareRevPB2)      
//                {
//                                Parameter_bank[6] = firmwareRevPB1;      //неисправна ячейка ЕЕПРОМ point1
//
//                }                                       
//        }    

//        
//        if(tmpcrc==firmwareCRCPB1)          //                              проверяем калибровочный коэффициент К
//        {
//                if(tmpcrc!=firmwareCRCPB2)//неисправна ячейка ЕЕПРОМ PB2         
//                        firmwareCRCPB2= tmpcrc;      
//        }
//        else
//        {
//                if(tmpcrc==firmwareCRCPB2)       
//                        firmwareCRCPB1 = tmpcrc;    //неисправна ячейка ЕЕПРОМ PB1
//                else if(firmwareCRCPB1==firmwareCRCPB2)
//                {
//
//                                tmpcrc =firmwareCRCPB1;//      неисправна ячейка ЕЕПРОМ point1
//                               
//                }                                       
//        }  
//        
//        if(tmpPar==devAddrPB1)                                        //проверяем калибровочный коэффициент B
//        {
//                if(tmpPar!=devAddrPB2)//неисправна ячейка ЕЕПРОМ PB2      
//                        devAddrPB2=tmpPar;      
//        }
//        else
//        {
//                if(tmpPar==devAddrPB2)   
//                        devAddrPB1 = tmpPar;    //неисправна ячейка ЕЕПРОМ PB1
//                else if(devAddrPB1==devAddrPB2)
//                {              
//                                Parameter_bank[25] = devAddrPB1;      //неисправна ячейка ЕЕПРОМ point1
//                               
//                }                                       
//        }    
     return 0;
...
//__eeprom char devAddrPB2;
// tmpcrc = (int)(Parameter_bank[10]<<8)|Parameter_bank[11];
//         if(Parameter_bank[2]==devCodePB1)                //проверяем 1ю точку калибровки выбранного диапазона
//        {
//                if(Parameter_bank[2]!=devCodePB2)//неисправна ячейка ЕЕПРОМ PB2 
//                        devCodePB2 = Parameter_bank[2];      
//        }
//        else
//        {
//                if(Parameter_bank[2]==devCodePB2)
//                        devCodePB1 = Parameter_bank[2];    //неисправна ячейка ЕЕПРОМ PB1
//                else if(devCodePB1==devCodePB2)      
//                {
//                                Parameter_bank[2] = devCodePB1;      //неисправна ячейка ЕЕПРОМ point1
//                               // CalculateCalibrationRates();
//                }                                       
//          }    
//          #asm("wdr")
...
//        if(Parameter_bank[25]==devAddrPB1)                                        //проверяем калибровочный коэффициент B
//        {
//                if(Parameter_bank[25]!=devAddrPB2)//неисправна ячейка ЕЕПРОМ PB2 
//                        devAddrPB2= Parameter_bank[25];      
//        }
//        else
//        {
//                if(Parameter_bank[25]==devAddrPB2)
//                        devAddrPB1 = Parameter_bank[25];    //неисправна ячейка ЕЕПРОМ PB1
//                else if(devAddrPB1==devAddrPB2)
//                {
//                                Parameter_bank[25] = devAddrPB1;      //неисправна ячейка ЕЕПРОМ point1
//                               
...
{                    
        ...
        {
                if(ADC_PV_calibration_point1[index]==ADC_PV_calibration_point1_PB2[index])
                        ADC_PV_calibration_point1_PB1[index] = ADC_PV_calibration_point1[index];    //неисправна ячейка ЕЕПРОМ PB1
                else if(ADC_PV_calibration_point1_PB1[index]==ADC_PV_calibration_point1_PB2[index])      
                {
                                ADC_PV_calibration_point1[index] = ADC_PV_calibration_point1_PB1[index];      //неисправна ячейка ЕЕПРОМ point1
                                CalculateCalibrationRates();
                }                                       
          }
        if(ADC_PV_calibration_point2[index]==ADC_PV_calibration_point2_PB1[index])                //проверяем 2ю точку калибровки выбранного диапазона
        ...
        {
                if(ADC_PV_calibration_point2[index]==ADC_PV_calibration_point2_PB2[index])
                        ADC_PV_calibration_point2_PB1[index] = ADC_PV_calibration_point2[index];    //неисправна ячейка ЕЕПРОМ PB1
                else if(ADC_PV_calibration_point2_PB1[index]==ADC_PV_calibration_point2_PB2[index])      
                {
                                ADC_PV_calibration_point2[index] = ADC_PV_calibration_point2_PB1[index];      //неисправна ячейка ЕЕПРОМ point1
                                CalculateCalibrationRates();
                }                                       
        }
        ...
        {
                if(calibrationKeep[index]==calibrationKeep_PB2[index])
                        calibrationKeep_PB1[index] = calibrationKeep[index];    //неисправна ячейка ЕЕПРОМ PB1
                else if(calibrationKeep_PB1[index]==calibrationKeep_PB2[index])
                {
                                calibrationKeep[index] = calibrationKeep_PB1[index];      //неисправна ячейка ЕЕПРОМ point1
                                CalculateCalibrationRates();
                }                                       
        }  
        ...
        {
                if(calibrationBeep[index]==calibrationBeep_PB2[index])
                        calibrationBeep_PB1[index] = calibrationBeep[index];    //неисправна ячейка ЕЕПРОМ PB1
                else if(calibrationBeep_PB1[index]==calibrationBeep_PB2[index])
                {
                                calibrationBeep[index] = calibrationBeep_PB1[index];      //неисправна ячейка ЕЕПРОМ point1
                                CalculateCalibrationRates();
                }                                       
        }           
}
...
{
   // if(j==0x1BDE)j+=2;//перескакиваем адрес с контрольной суммой для ПО загрузчика для ПК         
    if(j==0xDEF)j++;
